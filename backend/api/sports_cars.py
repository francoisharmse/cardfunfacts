...
router = APIRouter(prefix="/api/v1/sportscars", tags=["sportscars"])
...
async def list_sports_cars_images():
    """
    ...
    """
    try:
        minio_service = get_minio_service("sportscars")
        ...
        objects = minio_service.list_objects_from_bucket(
            "sportscars", prefix="", recursive=True
        )
        logger.info(f"Found {len(objects)} objects in 'sportscars' bucket")
        if objects:
        ...
        for obj in objects:
            ...
            if any(obj_name_lower.endswith(ext) for ext in image_extensions):
                ...
                public_url = minio_service.get_public_url(
                    obj["object_name"], bucket_name="sportscars"
                )
        ...
        logger.error(f"Error in list_sports_car_images: {str(e)}", exc_info=True)
        raise HTTPException(
...
async def get_sports_car_image(
    name: str = Query(..., description="Name of the image file"),
):
    """
    Get detailed information about a specific sports car image
    ...
    try:
        ...
        # Generate public URL
        public_url = minio_service.get_public_url(name, bucket_name="sportscars")
        ...
        logger.error(f"Error in get_sports_car_image: {str(e)}", exc_info=True)
        raise HTTPException(
